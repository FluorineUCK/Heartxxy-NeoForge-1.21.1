...
object OpBreed : SpellAction {
    ...
    override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
        ...
        if (target !is Animal || target.age < 0) throw MishapCantBreed(target, false)
        return SpellAction.Result(
            Spell(target), MediaConstants.SHARD_UNIT,
            listOf(ParticleSpray.cloud(target.position().add(0.0, target.eyeHeight / 2.0, 0.0), 1.0))
        )
    }

    private data class Spell(val target: Animal) : RenderedSpell {
